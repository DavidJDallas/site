export const alexanderPost = {
    ...
    content: `
        ...
        <p>
        I want to draw out something that seems to be a tension in the views put forward here, taken together. At first glance, it seems that Alexander is saying do not stick to existing paradigms, but start every problem afresh and find the best match for the context. And that Whitney is being far more cautious here and saying that existing paradigms work for a reason, and feel free to subvert them but only if you fully understand. Phrased slightly differently, that Alexander seems to advocate starting afresh with each problem, and that Whitney advises caution against the departure from normal form. 
        <br>
        To me, this is the wrong reading. The way I want to suggest reading the two taken together is that they are saying the same thing, but with Whitney giving a much clearer and well-emphasised version. The way I want to read Alexander's argument is that sticking to existing paradigms is fine, providing the context matches. I think micro-services is a good way to illustrate this. Micro-services is certainly a well-established paradigm that is incredibly useful for certain things in architecture. But it definitely has a time and a place. Using a micro-services architecture for a 1 person team trying to get a product off the ground is almost definitely adding unecessary complexity into your project; using a micro-service architecture for a 15 person team where there are several services with distinct, modular features is probably a smart choice, since it allows the develoeprs to split naturally into teams and work on these different services without stepping on each other's toes. They can build independently, scale independently, etc. 
        </p>
        <p>
